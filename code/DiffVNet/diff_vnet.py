...
class DiffVNet(nn.Module):
    ...
    def forward(self, image=None, x=None, pred_type=None, step=None, return_features=False):
        ...
        if pred_type == "q_sample":
            ...
            x1, x2, x3, x4, x5, temb = self.denoise_model.encoder(x, temb)
            out = self.decoder_theta(x1, x2, x3, x4, x5)
            if return_features:
                return out, x5
            return out
            ...
            x, temb = self.embedding(image)
            x1, x2, x3, x4, x5, temb = self.denoise_model.encoder(x, temb)
            out = self.decoder_psi(x1, x2, x3, x4, x5)
            if return_features:
                return out, x5
            return out
