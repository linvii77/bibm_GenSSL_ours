...
if __name__ == '__main__':
    ...
    if use_fused:
        fused_proxy = FusedProxyLoss(
            in_channels=config.n_filters * 16,
        ...
        optimizer.add_param_group({'params': fused_proxy.parameters()})
    iter_num = 0
    ...
    for epoch_num in range(args.max_epoch + 1):
        ...
        for batch_l, batch_u in tqdm(zip(labeled_loader, unlabeled_loader)):
            ...
            if args.mixed_precision:
                with autocast():
                    ...
                    if use_fused:
                        p_l_psi, feat_l = model(image=image_l, pred_type="D_psi_l", return_features=True)
                    else:
                    ...
                    if use_fused:
                        p_u_theta, feat_u = model(image=image_u, pred_type="D_theta_u", return_features=True)
                    else:
                    ...
                    if sac_active or cdba_active:
                        feat = torch.cat([feat_l, feat_u], dim=0)
                        label_l_fp = label_l.squeeze(1)
                        loss_cdba, loss_sac, fp_stats = fused_proxy(
                            feat, label_l_fp, labeled_bs=image_l.shape[0], iter_num=iter_num)
                        loss_fp = torch.zeros(1, device=loss.device)
                        if sac_active:
                            loss_fp = loss_fp + args.lambda_sac * loss_sac
                        if cdba_active:
                            loss_fp = loss_fp + args.lambda_cdba * loss_cdba
                        loss = loss + loss_fp
                        loss_fp_list.append(loss_fp.item())
        ...
        if use_fused and loss_fp_list:
            writer.add_scalar('loss/fused_proxy', np.mean(loss_fp_list), epoch_num)
        ...
        new_lr = poly_lr(epoch_num, args.max_epoch, args.base_lr, 0.9)
