...
class RandomCrop(AbstractTransform):
    ...
    50% of the time, centers the crop on a random foreground voxel so that
    rare small structures (eso, rad, lad) are seen during every other patch.
    Args:
    output_size (int): Desired output size
    """

    def __init__(self, output_size, fg_prob=0.5):
        self.output_size = output_size
    ...
    def __call__(self, **data_dict):
        ...
        # 50% foreground-centred crop: pick a random foreground voxel and
        # centre the patch on it (clamped to valid range).
        fg_idx = np.argwhere(label > 0)
        if np.random.random() < self.fg_prob and len(fg_idx) > 0:
            center = fg_idx[np.random.randint(len(fg_idx))]
            w1 = int(np.clip(center[0] - self.output_size[0] // 2, 0, w - self.output_size[0]))
            h1 = int(np.clip(center[1] - self.output_size[1] // 2, 0, h - self.output_size[1]))
            d1 = int(np.clip(center[2] - self.output_size[2] // 2, 0, d - self.output_size[2]))
        else:
